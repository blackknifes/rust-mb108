    ...
    let init_block = if !attrs.is_empty() {
        ...
        if let Meta::NameValue(value) = meta {
            if !value.path.is_ident("dll") {
                panic!("mb108::main is only support to dll");
            }
        ...
        quote! {
            ...
            fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
                ...
                localSet.block_on(&runtime, async {
                    ...
                    loop {
                        match mb108::run_once() {
                            mb108::RunOnceFlag::Idle => tokio::task::yield_now().await,
                            mb108::RunOnceFlag::Exit => break,
                            _ => continue,
                        }
                    }
